...
Zip_code is stored as VARCHAR(5) instead of INT because ZIP/postal codes are not used for calculations.
...
yes TINYINT isa god choice because it uses less storage space than INT.

-- Q4: If you add a CHECK constraint that rating must be BETWEEN 1 AND 5, what happens if you try to INSERT rating = 0?
An error message will appear because 0 does not satisfy the CHECK condition.

-- Q5: Why does sales.staffs have UNIQUE constraint on email but not on phone?

Email must be unique for each staff member, while phone numbers may be shared or changed.
